...
async function updateTursoStock() {
  const client = createClient({ url: TURSO_URL, authToken: TURSO_TOKEN });
  await client.execute({
    sql: "UPDATE Stock SET amount = 20, updatedAt = ? WHERE product = 'ECEYREKTL' AND dealerId = 'merkez';",
    args: [new Date().toISOString()]
  });
  const res = await client.execute("SELECT product, label, amount FROM Stock WHERE product = 'ECEYREKTL';");
  console.log('✓ Turso Stock updated successfully:', res.rows);
}

updateTursoStock().catch(console.error);
