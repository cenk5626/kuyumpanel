...
async function testList() {
  ...
  const models = ['gemini-3.5-flash', 'gemini-3.6-flash', 'gemini-3.1-flash-lite', 'gemini-2.5-flash-lite'];
  ...
  for (const m of models) {
    ...
    try {
      ...
      console.log(`[${m}] Status: ${response.status} in ${duration}ms! Answer:`, data.candidates?.[0]?.content?.parts?.[0]?.text?.substring(0, 70));
    } catch (e) {
