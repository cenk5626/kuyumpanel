...
import { X, Printer, ArrowRight, ShieldCheck } from 'lucide-react';
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white text-zinc-900 rounded-2xl shadow-2xl print:shadow-none print:max-w-none print:w-full print:h-auto print:rounded-none">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 print:hidden bg-zinc-50 rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm text-zinc-800">Şubeler Arası Transfer Sevk İrsaliyesi</span>
          </div>
        ...
        <div className="p-8 font-sans space-y-6 print:p-6 print:space-y-4">
          ...
          <div className="flex items-start justify-between border-b pb-6">
            ...
            <div className="text-right">
              ...
              <div className="mt-1 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-xs font-semibold bg-zinc-100 text-zinc-800">
          ...
          <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-zinc-50 border border-zinc-200">
            <div>
              ...
              <p className="text-base font-bold text-zinc-900 mt-1">{transfer.fromBranch?.name}</p>
              <p className="text-xs font-mono text-zinc-600">Kod: {transfer.fromBranch?.code}</p>
              {transfer.fromBranch?.phone && (
                <p className="text-xs text-zinc-500 mt-0.5">Tel: {transfer.fromBranch.phone}</p>
              )}
            </div>
            <div className="border-l border-zinc-200 pl-4">
              ...
              <p className="text-base font-bold text-zinc-900 mt-1">{transfer.toBranch?.name}</p>
              <p className="text-xs font-mono text-zinc-600">Kod: {transfer.toBranch?.code}</p>
              {transfer.toBranch?.phone && (
                <p className="text-xs text-zinc-500 mt-0.5">Tel: {transfer.toBranch.phone}</p>
              )}
            </div>
          </div>
              ...
              <tbody className="divide-y divide-zinc-200 text-xs">
                {transfer.lines?.map((line: any, idx: number) => (
                  <tr key={line.id || idx}>
                    ...
                    <td className="py-2 px-3 font-mono font-bold text-zinc-900">{line.barcode}</td>
                    <td className="py-2 px-3 text-zinc-800 font-medium">{line.productTitle}</td>
                    <td className="py-2 px-3 text-center text-zinc-600">{line.carat ? `${line.carat}K` : '-'}</td>
                    <td className="py-2 px-3 text-right font-mono font-bold text-zinc-900">
                      {Number(line.weight || 0).toFixed(2)} gr
                    </td>
                    <td className="py-2 px-3 text-center font-bold text-zinc-900">{line.quantity} Adet</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
